...
class RingBufferTest {
    ...
    void testEnqueue(int referenceSize, String... bufferElements) {
        for (int i = 0; i < bufferElements.length; i++) {
            ...
            if (i > RING_BUFFER_CAPACITY) {
                ...
                assertEquals(bufferElements[i - RING_BUFFER_CAPACITY + 1], ringBuffer.peek());
            } else {
    ...
    @ParameterizedTest
    @MethodSource("provideDifferentSizes")
    void testPeek(int referenceSize, String... bufferElements) {
        // DONE: implement this test yourself
        for (int i = 0; i < bufferElements.length; i++) {
            ringBuffer.enqueue(bufferElements[i]);
            if (i >= RING_BUFFER_CAPACITY) {
                assertTrue(ringBuffer.peek().equalsIgnoreCase(bufferElements[i - RING_BUFFER_CAPACITY + 1]));
            } else {
                assertTrue(ringBuffer.peek().equalsIgnoreCase(bufferElements[0]));
            }
        }
